  ...
  PROCESS_BEGIN();
	for (;;) {
		...
		if(err == USCXML_ERR_DONE){
			...
			PROCESS_WAIT_UNTIL(rootMachine.hasPendingWork() || ev == StateMachine::ce_scxml);
  ...
  PROCESS_BEGIN();
	for (;;) {
		...
		if(err == USCXML_ERR_DONE){
			...
			PROCESS_WAIT_UNTIL(ptr_invoked_machine->hasPendingWork() || ev == StateMachine::ce_scxml);
			//printf("moved ahead of wait.\n");
