...
namespace uscxml {
    ...
    void ChartToVHDL::writeTestbench(std::ostream& stream) {
        ...
        stream << "  clk   <= not clk  after 20 ns;  -- 25 MHz Taktfrequenz" << std::endl;
        stream << "  reset <= '1', '0' after 100 ns; -- erzeugt Resetsignal: --__" << std::endl;
        stream << " " << std::endl;
        stream << "  -- Modulinstatziierung" << std::endl;
...
void ChartToVHDL::writeTopDown(std::ostream & stream) {
    ...
    void ChartToVHDL::writeActiveStateNplusOne(std::ostream & stream) {
        ...
        for (size_t i = 0; i < _states.size(); i++) {
            ...
            //		                 "not is_active_"+toStr(i)+"_sig or in_exit_set_"+toStr(i)+"_sig");
            //
            //		tree->print(stream);
            //		stream << ";" << std::endl;
        }
    ...
    void ChartToVHDL::writeEntrySet(std::ostream & stream) {
        ...
        for (size_t i = 0; i < _states.size(); i++) {
            ...
            //TODO use makros
            //		VBranch* tree = (VASSIGN ,
            //		                 VLINE("in_entry_set" + toStr(i) + "_sig"),
            //		                 VLINE("in_complete_entry_set_"+toStr(i)+"_sig"),
            //		                 "is_active_"+toStr(i)+"_sig or not in_exit_set_"+toStr(i)+"_sig");
            //
            //		tree->print(stream);
            //		stream << ";" << std::endl;
        }
