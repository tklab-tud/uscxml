...
<xsl:template match="//@conf:datamodel">
	<xsl:attribute name="datamodel">lua</xsl:attribute>
</xsl:template>
...
<xsl:template match="//@conf:emptyEventData">
	<xsl:attribute name="cond">_event.data == nil</xsl:attribute>
</xsl:template>
...
<xsl:template match="//@conf:varPrefix">
		...
		<xsl:analyze-string select="."
			regex="(\w+)(\W)(\w+)">
					...
					<!-- string.sub(Var1,1,string.len(Var2))==Var2 -->
					<!-- </xsl:text>Var<xsl:value-of select="regex-group(3)"/><xsl:text> -->
					<!-- </xsl:text>Var<xsl:value-of select="regex-group(1)"/><xsl:text> -->
					<xsl:text>string.sub(</xsl:text>Var<xsl:value-of select="regex-group(3)"/><xsl:text>,1,string.len(</xsl:text>Var<xsl:value-of select="regex-group(1)"/><xsl:text>))==</xsl:text>Var<xsl:value-of select="regex-group(1)"/><xsl:text></xsl:text>
...
<xsl:template match="//@conf:isBound">
	<xsl:attribute name="cond">Var<xsl:value-of select="." /> ~= nil</xsl:attribute>
</xsl:template>
...
<xsl:template match="//@conf:systemVarIsBound">
	<xsl:attribute name="cond"><xsl:value-of select="." /> ~= nil</xsl:attribute>
</xsl:template>
...
<xsl:template match="conf:extendArray">
	<assign xmlns="http://www.w3.org/2005/07/scxml">
	  <xsl:attribute name="location">Var<xsl:value-of select="@id"/>[4]</xsl:attribute>
	  <xsl:attribute name="expr">'asdf'</xsl:attribute>
	  </assign>
	</xsl:template>
...
<xsl:value-of select="$targetArray"/>[0][0]== <xsl:value-of select="$value"/> and <xsl:value-of select="$targetArray"/>[0][1] == <xsl:value-of select="$value"/>  and <xsl:value-of select="$targetArray"/>[1][0] == <xsl:value-of select="$value"/> and <xsl:value-of select="$targetArray"/>[1][1] == <xsl:value-of select="$value"/>
</xsl:matching-substring>
...
<xsl:template match="//@conf:methodIsPost">
 <xsl:attribute name="cond">string.find(_event.raw, 'POST')</xsl:attribute>
</xsl:template>
...
<xsl:matching-substring>string.find(_event.raw, 'Var<xsl:value-of select="regex-group(1)"/>=<xsl:value-of select="regex-group(3)"/>')</xsl:matching-substring></xsl:analyze-string></xsl:attribute>
</xsl:template>

<!-- returns true if _event/raw contains the param with the specified value -->
<xsl:template match="//@conf:eventNamedParamHasValue">
 <xsl:attribute name="cond"><xsl:analyze-string select="." regex="(\S+)(\s+)(\S+)">
 <xsl:matching-substring>string.find(_event.raw, '<xsl:value-of select="regex-group(1)"/>=<xsl:value-of select="regex-group(3)"/>')</xsl:matching-substring></xsl:analyze-string></xsl:attribute>
</xsl:template>

<xsl:template match="//@conf:messageBodyEquals">
 <xsl:attribute name="cond">string.find(_event.raw, '<xsl:value-of select="."/>')</xsl:attribute>
</xsl:template>
