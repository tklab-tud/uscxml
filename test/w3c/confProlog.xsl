...
<xsl:template match="//@conf:datamodel">
	<xsl:attribute name="datamodel">prolog</xsl:attribute>
</xsl:template>
...
<!-- var1(X), Y is 5 + X, retractall(var1(X)), assert(var1(Y)) -->
<xsl:template match="conf:incrementID">
	<assign xmlns="http://www.w3.org/2005/07/scxml">
	  <xsl:attribute name="location">var<xsl:value-of select="@id"/>(Y)</xsl:attribute>
	  <xsl:attribute name="expr">var<xsl:value-of select="@id"/>(X), Y is X + 1, retractall(var1(_))</xsl:attribute>
	  </assign>
	</xsl:template>
		...
		<xsl:analyze-string select="."
			regex="([0-9]+)([=&lt;&gt;]=?)(.*)">
					<xsl:matching-substring>
						...
						<xsl:choose>
							<xsl:when test="$op='='">X = <xsl:value-of select="regex-group(3)"/>, var<xsl:value-of select="regex-group(1)"/>(X)</xsl:when>
							<xsl:otherwise>var<xsl:value-of select="regex-group(1)"/><xsl:value-of select="$op"/><xsl:value-of select="regex-group(3)"/>
							</xsl:otherwise>
					 </xsl:choose>
					</xsl:matching-substring>
...
<xsl:template match="//@conf:eventLanguageMatchesDatamodel">
	<xsl:attribute name="cond"> _event.language == 'prolog'</xsl:attribute>
</xsl:template>
...
<xsl:template match="//@conf:arrayVar">
	<xsl:attribute name="array">var<xsl:value-of select="." />(X), member(M, X)</xsl:attribute>
</xsl:template>
